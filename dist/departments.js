export const departments = [
  ...
  ['2A','Corse-du-Sud'],['2B','Haute-Corse'],['21','Côte-d’Or'],['22','Côtes-d’Armor'],['23','Creuse'],['24','Dordogne'],['25','Doubs'],['26','Drôme'],['27','Eure'],['28','Eure-et-Loir'],['29','Finistère'],
...
const normalized = (value = '') => value.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[’']/g, '-').toLowerCase().trim();

export const departmentLabel = (code) => {
  const item = departments.find(([value]) => value === String(code || '').toUpperCase());
  return item ? `${item[0]} - ${item[1]}` : String(code || '');
};

export const departmentCode = (input) => {
  const value = normalized(String(input || ''));
  const code = value.split(/\s*-\s*/)[0].toUpperCase();
  const byCode = departments.find(([itemCode]) => itemCode === code);
  if (byCode) return byCode[0];
  const exactName = departments.find(([, name]) => normalized(name) === value);
  return exactName?.[0] || null;
};

export const attachDepartmentPicker = (input) => {
  if (!input) return;
  const id = `${input.id || 'department'}-list`;
  let list = document.getElementById(id);
  if (!list) {
    list = document.createElement('datalist');
    list.id = id;
    list.innerHTML = departments.map(([code, name]) => `<option value="${code} - ${name}"></option>`).join('');
    document.body.append(list);
  }
  input.setAttribute('list', id);
  input.setAttribute('placeholder', 'Numéro ou nom du département');
  input.removeAttribute('maxlength');
  input.addEventListener('change', () => {
    const code = departmentCode(input.value);
    if (code) input.value = departmentLabel(code);
  });
};
